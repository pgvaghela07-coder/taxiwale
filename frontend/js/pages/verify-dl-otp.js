...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const storedOTP = sessionStorage.getItem("dlOTP");
  const storedMobile = sessionStorage.getItem("dlOTPMobile");

  // Clear sessionStorage immediately (user has already seen the OTP on previous page)
  if (storedOTP) {
    sessionStorage.removeItem("dlOTP");
  }
  if (storedMobile) {
    sessionStorage.removeItem("dlOTPMobile");
  }
  ...
  otpForm.addEventListener("submit", async (e) => {
    ...
    try {
      // Verify DL OTP
      await verifyDLOTP(otp);
    } catch (error) {
      console.error("DL OTP verification error:", error);
  ...
  // Verify DL OTP
  async function verifyDLOTP(otp) {
    try {
      // Get stored DL number
      const dlNumber = sessionStorage.getItem("pendingDLNumber");
      if (!dlNumber) {
        throw new Error("Driving License number not found. Please start again.");
      }
      ...
      const response = await apiService.request("/verification/driving-license", {
        method: "POST",
        body: JSON.stringify({
          dlNumber: dlNumber,
          otp: otp
      ...
      sessionStorage.setItem("dlVerified", "true");

      // Clear pending DL number
      sessionStorage.removeItem("pendingDLNumber");

      // Get updated user data
      const userData = JSON.parse(localStorage.getItem("user") || "{}");
      
      // Check if Aadhaar is also verified
      const aadhaarVerified = userData.verificationStatus?.aadhaar?.verified || false;
