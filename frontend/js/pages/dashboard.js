...
async function loadVehiclesFromAPI() {
  console.log("🔄 [loadVehiclesFromAPI] Starting to load vehicles...");
  const container = document.getElementById("vehiclesCards");
  if (!container) {
    console.error("❌ [loadVehiclesFromAPI] vehiclesCards container not found");
    return;
  }
  console.log("✅ [loadVehiclesFromAPI] Container found");
  ...
  try {
    // Check if API service is available
    if (typeof apiService === "undefined") {
      console.error("❌ [loadVehiclesFromAPI] API service not loaded");
      throw new Error("API service not loaded");
    }
    console.log("✅ [loadVehiclesFromAPI] API service available");

    // Fetch all available vehicles (all users' active vehicles)
    const filters = {}; // Can add filters later if needed
    console.log("📤 [loadVehiclesFromAPI] Calling apiService.getVehicles with filters:", filters);
    const response = await apiService.getVehicles(filters);
    console.log("🚗 [loadVehiclesFromAPI] Full API Response:", response);
    console.log("🚗 [loadVehiclesFromAPI] Response type:", typeof response);
    console.log("🚗 [loadVehiclesFromAPI] Is array:", Array.isArray(response));

    // Handle different response formats
    let vehicles = [];
    if (Array.isArray(response)) {
      vehicles = response;
      console.log("✅ [loadVehiclesFromAPI] Response is direct array, count:", vehicles.length);
    } else if (response && response.data && Array.isArray(response.data)) {
      vehicles = response.data;
      console.log("✅ [loadVehiclesFromAPI] Response has data array, count:", vehicles.length);
    } else if (response && response.vehicles && Array.isArray(response.vehicles)) {
      vehicles = response.vehicles;
      console.log("✅ [loadVehiclesFromAPI] Response has vehicles array, count:", vehicles.length);
    } else {
      console.warn("⚠️ [loadVehiclesFromAPI] Unexpected response format:", response);
    }

    console.log("🚗 [loadVehiclesFromAPI] Vehicles extracted:", vehicles.length, vehicles);

    // Store vehicles in Map for WhatsApp interest functionality
    dashboardVehiclesMap.clear(); // Clear previous data
    vehicles.forEach((vehicle) => {
      if (vehicle._id) {
        // Convert _id to string to ensure consistent key format
        const vehicleIdKey = vehicle._id.toString
          ? vehicle._id.toString()
          : String(vehicle._id);
        dashboardVehiclesMap.set(vehicleIdKey, vehicle);
      }
    });
    console.log("💾 Stored", dashboardVehiclesMap.size, "vehicles in Map");

    if (vehicles.length === 0) {
      container.innerHTML = `
        <div class="no-bookings" style="text-align: center; padding: 60px 20px; color: #bdbdbd;">
          <div style="font-size: 64px; margin-bottom: 16px;">🚗</div>
          <h3 style="margin-bottom: 8px; color: #757575;">No vehicles available</h3>
          <p>You don't have any free vehicles posted yet</p>
        </div>
      `;
      return;
    }
    ...
    console.log("🎨 [loadVehiclesFromAPI] Rendering", vehicles.length, "vehicle cards");
    try {
      const cardsHTML = vehicles
        .map((vehicle, index) => {
          try {
            ...
            console.error(`❌ [loadVehiclesFromAPI] Error creating vehicle card ${index}:`, cardError, vehicle);
      ...
      console.log("✅ [loadVehiclesFromAPI] Successfully created", cardsHTML.split('class="vehicle-manage-card').length - 1, "vehicle cards");
      container.innerHTML = cardsHTML;
      
      if (cardsHTML.trim() === "") {
        console.warn("⚠️ [loadVehiclesFromAPI] No valid vehicle cards were created");
        container.innerHTML = `
          <div class="no-bookings" style="text-align: center; padding: 60px 20px; color: #bdbdbd;">
            <div style="font-size: 64px; margin-bottom: 16px;">🚗</div>
            <h3 style="margin-bottom: 8px; color: #757575;">No vehicles available</h3>
            <p>Check back later for new vehicle availability</p>
          </div>
        `;
      }
    } catch (renderError) {
      console.error("❌ [loadVehiclesFromAPI] Error rendering vehicle cards:", renderError);
      throw renderError;
    }
  } catch (error) {
    console.error("❌ [loadVehiclesFromAPI] Error loading vehicles:", error);
    console.error("❌ [loadVehiclesFromAPI] Error details:", {
      message: error.message,
      stack: error.stack,
    ...
    container.innerHTML = `
      <div class="error-bookings" style="text-align: center; padding: 60px 20px; color: #f44336;">
        ...
        <button onclick="loadVehiclesFromAPI()" style="padding: 12px 24px; background: #ff9900; color: white; border: none; border-radius: 8px; cursor: pointer; font-size: 16px;">
          Retry
        </button>
      </div>
    `;
...
function createBookingAvailableContent() {
  return `
    ...
    <div class="view-switcher">
      ...
      <button class="view-btn" data-view="vehicles" onclick="switchView('vehicles')">
        🚗 Free Vehicles
      </button>
    </div>
    ...
    <!-- Free Vehicles View -->
    <div class="vehicles-view hidden" id="vehiclesView">
      <!-- Title Section -->
      <div class="vehicles-title-section">
        <h2 class="vehicles-title">Available Cars / Drivers</h2>
        <p class="vehicles-subtitle">Find vehicles currently available for booking</p>
      </div>

      <!-- Vehicles Filter Bar -->
      <div class="compact-filter-bar vehicles-filter-bar">
        <div class="quick-chips-container">
          <button class="filter-chip quick-filter active" data-filter="all" onclick="filterVehicles('all')">All</button>
          <button class="filter-chip quick-filter" data-filter="verified" onclick="filterVehicles('verified')">✓ Verified</button>
          <button class="filter-chip quick-filter" data-filter="now" onclick="filterVehicles('now')">Available Now</button>
        </div>
      </div>

      <!-- Vehicles Cards Container -->
      <div class="vehicles-cards-container" id="vehiclesCards">
        <!-- Vehicle Card 1 -->
        <div class="vehicle-card" data-vehicle="suv" data-verified="true" data-availability="now">
          <div class="card-header">
            <div class="user-info">
              <div class="user-avatar">
                <img src="https://ui-avatars.com/api/?name=Sunil+Patel&background=ff9900&color=1a1a1a" alt="Driver">
              </div>
              <div class="user-details">
                <div class="user-name-row">
                  <h4 class="user-name">Sunil Patel</h4>
                  <span class="verified-badge">✓ Verified</span>
                </div>
                <div class="id-rating-row">
                  <p class="vehicle-location">📍 Ahmedabad</p>
                  <div class="rating-section">
                    <span class="gold-stars">⭐⭐⭐⭐⭐</span>
          ...
          <div class="card-body">
            <div class="availability-badge active-now">
            ...
            <div class="vehicle-info-container">
              <div class="vehicle-main-info">
                ...
                <div class="vehicle-details">
                  <p class="vehicle-name">Toyota Innova</p>
                  <div class="vehicle-specs">
                    <span class="spec-chip">⚡ Petrol</span>
                    <span class="spec-chip">👥 7 Seats</span>
                    <span class="spec-chip">❄️ AC</span>
                  </div>
                </div>
              </div>
            </div>

            <div class="vehicle-notes-card">
              <span class="notes-label">About:</span>
              <span>Professional driver with 8 years experience. Clean and well-maintained vehicle.</span>
            </div>
          </div>

          <div class="card-footer">
            <button class="action-btn whatsapp-btn" onclick="contactWhatsApp('919876543220')">
              <span class="btn-icon">💬</span>
              <span>WhatsApp</span>
            </button>
            <button class="action-btn call-btn" onclick="contactCall('919876543220')">
              <span class="btn-icon">📞</span>
              <span>Call</span>
            </button>
          </div>
        ...
        <!-- Vehicle Card 2 -->
        <div class="vehicle-card" data-vehicle="sedan" data-verified="true" data-availability="now">
          <div class="card-header">
            <div class="user-info">
              <div class="user-avatar">
                <img src="https://ui-avatars.com/api/?name=Ramesh+Shah&background=ff9900&color=1a1a1a" alt="Driver">
              </div>
              <div class="user-details">
                <div class="user-name-row">
                  <h4 class="user-name">Ramesh Shah</h4>
                  <span class="verified-badge">✓ Verified</span>
                </div>
                <div class="id-rating-row">
                  <p class="vehicle-location">📍 Vadodara</p>
                  <div class="rating-section">
                    <span class="gold-stars">⭐⭐⭐⭐⭐</span>
          ...
          <div class="card-body">
            ...
            <div class="vehicle-info-container">
              <div class="vehicle-main-info">
                <span class="vehicle-type-icon">🚗</span>
                <div class="vehicle-details">
                  <p class="vehicle-name">Honda City</p>
                  <div class="vehicle-specs">
                    <span class="spec-chip">⚡ Diesel</span>
                    <span class="spec-chip">👥 5 Seats</span>
                    <span class="spec-chip">❄️ AC</span>
                  </div>
                </div>
              </div>
            ...
            <div class="vehicle-notes-card">
              <span class="notes-label">About:</span>
              <span>Experienced driver, vehicle in excellent condition. Available for both short and long trips.</span>
            </div>
          </div>

          <div class="card-footer">
            <button class="action-btn whatsapp-btn" onclick="contactWhatsApp('919876543221')">
              <span class="btn-icon">💬</span>
              <span>WhatsApp</span>
            </button>
            <button class="action-btn call-btn" onclick="contactCall('919876543221')">
              <span class="btn-icon">📞</span>
              <span>Call</span>
            </button>
          </div>
        ...
        <!-- Vehicle Card 3 -->
        <div class="vehicle-card" data-vehicle="hatchback" data-verified="true" data-availability="later">
          <div class="card-header">
            <div class="user-info">
              <div class="user-avatar">
                <img src="https://ui-avatars.com/api/?name=Laxman+Pandya&background=ff9900&color=1a1a1a" alt="Driver">
              </div>
              <div class="user-details">
                <div class="user-name-row">
                  <h4 class="user-name">Laxman Pandya</h4>
                  <span class="verified-badge">✓ Verified</span>
                </div>
                <div class="id-rating-row">
                  <p class="vehicle-location">📍 Rajkot</p>
                  <div class="rating-section">
                    <span class="gold-stars">⭐⭐⭐⭐</span>
          ...
          <div class="card-body">
            ...
            <div class="vehicle-info-container">
              <div class="vehicle-main-info">
                <span class="vehicle-type-icon">🚗</span>
                <div class="vehicle-details">
                  <p class="vehicle-name">Maruti Swift</p>
                  <div class="vehicle-specs">
                    <span class="spec-chip">⚡ CNG</span>
                    <span class="spec-chip">👥 5 Seats</span>
                    <span class="spec-chip">❄️ AC</span>
                  </div>
                </div>
            ...
            <div class="vehicle-notes-card">
              <span class="notes-label">About:</span>
              <span>Fuel-efficient vehicle, perfect for city commutes. Clean and comfortable.</span>
            </div>
          </div>

          <div class="card-footer">
            <button class="action-btn whatsapp-btn" onclick="contactWhatsApp('919876543222')">
              <span class="btn-icon">💬</span>
              <span>WhatsApp</span>
            </button>
            <button class="action-btn call-btn" onclick="contactCall('919876543222')">
              <span class="btn-icon">📞</span>
              <span>Call</span>
            </button>
          </div>
        ...
        <!-- Vehicle Card 4 -->
        <div class="vehicle-card" data-vehicle="luxury" data-verified="true" data-availability="now">
          <div class="card-header">
            <div class="user-info">
              <div class="user-avatar">
                <img src="https://ui-avatars.com/api/?name=Harsh+Trivedi&background=ff9900&color=1a1a1a" alt="Driver">
              </div>
              <div class="user-details">
                <div class="user-name-row">
                  <h4 class="user-name">Harsh Trivedi</h4>
                  <span class="verified-badge">✓ Verified</span>
                </div>
                <div class="id-rating-row">
                  <p class="vehicle-location">📍 Surat</p>
                  <div class="rating-section">
                    <span class="gold-stars">⭐⭐⭐⭐⭐</span>
          ...
          <div class="card-body">
            ...
            <div class="vehicle-info-container">
              <div class="vehicle-main-info">
                <span class="vehicle-type-icon">⭐</span>
                <div class="vehicle-details">
                  <p class="vehicle-name">Mercedes-Benz E-Class</p>
                  <div class="vehicle-specs">
                    <span class="spec-chip">⚡ Diesel</span>
                    <span class="spec-chip">👥 4 Seats</span>
                    <span class="spec-chip">❄️ AC</span>
                  </div>
                </div>
              </div>
            ...
            <div class="vehicle-notes-card">
              <span class="notes-label">About:</span>
              <span>Premium luxury vehicle with professional chauffeur. Perfect for VIPs and special occasions.</span>
            </div>
          </div>

          <div class="card-footer">
            <button class="action-btn whatsapp-btn" onclick="contactWhatsApp('919876543223')">
              <span class="btn-icon">💬</span>
              <span>WhatsApp</span>
            </button>
            <button class="action-btn call-btn" onclick="contactCall('919876543223')">
              <span class="btn-icon">📞</span>
              <span>Call</span>
            </button>
          </div>
        ...
        <div class="vehicle-card" data-vehicle="suv" data-verified="true" data-availability="now">
          <div class="card-header">
            <div class="user-info">
              <div class="user-avatar">
                <img src="https://ui-avatars.com/api/?name=Kiran+Patel&background=ff9900&color=1a1a1a" alt="Driver">
              </div>
              <div class="user-details">
                <div class="user-name-row">
                  <h4 class="user-name">Kiran Patel</h4>
                  <span class="verified-badge">✓ Verified</span>
                </div>
                <div class="id-rating-row">
                  <p class="vehicle-location">📍 Anand</p>
                  <div class="rating-section">
                    <span class="gold-stars">⭐⭐⭐⭐⭐</span>
          ...
          <div class="card-body">
            ...
            <div class="vehicle-info-container">
              <div class="vehicle-main-info">
                <span class="vehicle-type-icon">🚙</span>
                <div class="vehicle-details">
                  <p class="vehicle-name">Toyota Fortuner</p>
                  <div class="vehicle-specs">
                    <span class="spec-chip">⚡ Diesel</span>
                    <span class="spec-chip">👥 7 Seats</span>
                    <span class="spec-chip">❄️ AC</span>
            ...
            <div class="vehicle-notes-card">
              <span class="notes-label">About:</span>
              <span>Spacious SUV perfect for large families. Well-maintained with experienced driver.</span>
            </div>
          </div>

          <div class="card-footer">
            <button class="action-btn whatsapp-btn" onclick="contactWhatsApp('919876543224')">
              <span class="btn-icon">💬</span>
              <span>WhatsApp</span>
            </button>
            <button class="action-btn call-btn" onclick="contactCall('919876543224')">
              <span class="btn-icon">📞</span>
              <span>Call</span>
            </button>
          </div>
        ...
        <!-- Vehicle Card 6 -->
        <div class="vehicle-card" data-vehicle="sedan" data-verified="true" data-availability="later">
          <div class="card-header">
            <div class="user-info">
              <div class="user-avatar">
                <img src="https://ui-avatars.com/api/?name=Suresh+Desai&background=ff9900&color=1a1a1a" alt="Driver">
              </div>
              <div class="user-details">
                <div class="user-name-row">
                  <h4 class="user-name">Suresh Desai</h4>
                  <span class="verified-badge">✓ Verified</span>
                </div>
                <div class="id-rating-row">
                  <p class="vehicle-location">📍 Jamnagar</p>
                  <div class="rating-section">
                    <span class="gold-stars">⭐⭐⭐⭐</span>
          ...
          <div class="card-body">
            ...
            <div class="vehicle-info-container">
              <div class="vehicle-main-info">
                <span class="vehicle-type-icon">🚗</span>
                <div class="vehicle-details">
                  <p class="vehicle-name">Hyundai Verna</p>
                  <div class="vehicle-specs">
                    <span class="spec-chip">⚡ Petrol</span>
                    <span class="spec-chip">👥 5 Seats</span>
                    <span class="spec-chip">❄️ AC</span>
            ...
            <div class="vehicle-notes-card">
              <span class="notes-label">About:</span>
              <span>Comfortable sedan with modern features. Reliable driver for all types of journeys.</span>
            </div>
          </div>

          <div class="card-footer">
            <button class="action-btn whatsapp-btn" onclick="contactWhatsApp('919876543225')">
              <span class="btn-icon">💬</span>
              <span>WhatsApp</span>
            </button>
            <button class="action-btn call-btn" onclick="contactCall('919876543225')">
              <span class="btn-icon">📞</span>
              <span>Call</span>
            </button>
          </div>
        ...
        <!-- Vehicle Card 7 -->
        <div class="vehicle-card" data-vehicle="hatchback" data-verified="true" data-availability="now">
          <div class="card-header">
            <div class="user-info">
              <div class="user-avatar">
                <img src="https://ui-avatars.com/api/?name=Mohan+Sharma&background=ff9900&color=1a1a1a" alt="Driver">
              </div>
              <div class="user-details">
                <div class="user-name-row">
                  <h4 class="user-name">Mohan Sharma</h4>
                  <span class="verified-badge">✓ Verified</span>
                </div>
                <div class="id-rating-row">
                  <p class="vehicle-location">📍 Gandhinagar</p>
                  <div class="rating-section">
                    <span class="gold-stars">⭐⭐⭐⭐⭐</span>
          ...
          <div class="card-body">
            ...
            <div class="vehicle-info-container">
              <div class="vehicle-main-info">
                <span class="vehicle-type-icon">🚗</span>
                <div class="vehicle-details">
                  <p class="vehicle-name">Tata Tiago</p>
                  <div class="vehicle-specs">
                    <span class="spec-chip">⚡ Petrol</span>
                    <span class="spec-chip">👥 5 Seats</span>
                    <span class="spec-chip">❄️ AC</span>
            ...
            <div class="vehicle-notes-card">
              <span class="notes-label">About:</span>
              <span>Compact and efficient vehicle ideal for city travel. Excellent condition and friendly driver.</span>
            </div>
          </div>

          <div class="card-footer">
            <button class="action-btn whatsapp-btn" onclick="contactWhatsApp('919876543226')">
              <span class="btn-icon">💬</span>
              <span>WhatsApp</span>
            </button>
            <button class="action-btn call-btn" onclick="contactCall('919876543226')">
              <span class="btn-icon">📞</span>
              <span>Call</span>
            </button>
          </div>
        ...
        <div class="vehicle-card" data-vehicle="suv" data-verified="true" data-availability="now">
          <div class="card-header">
            <div class="user-info">
              <div class="user-avatar">
                <img src="https://ui-avatars.com/api/?name=Jagdish+Mehta&background=ff9900&color=1a1a1a" alt="Driver">
              </div>
              <div class="user-details">
                <div class="user-name-row">
                  <h4 class="user-name">Jagdish Mehta</h4>
                  <span class="verified-badge">✓ Verified</span>
                </div>
                <div class="id-rating-row">
                  <p class="vehicle-location">📍 Bhavnagar</p>
                  <div class="rating-section">
                    <span class="gold-stars">⭐⭐⭐⭐</span>
          ...
          <div class="card-body">
            ...
            <div class="vehicle-info-container">
              <div class="vehicle-main-info">
                <span class="vehicle-type-icon">🚙</span>
                <div class="vehicle-details">
                  <p class="vehicle-name">Mahindra Scorpio</p>
                  <div class="vehicle-specs">
                    <span class="spec-chip">⚡ Diesel</span>
                    <span class="spec-chip">👥 7 Seats</span>
                    <span class="spec-chip">❄️ AC</span>
            ...
            <div class="vehicle-notes-card">
              <span class="notes-label">About:</span>
              <span>Rugged SUV ideal for long distance travel. Experienced driver with excellent driving record.</span>
            </div>
          </div>

          <div class="card-footer">
            <button class="action-btn whatsapp-btn" onclick="contactWhatsApp('919876543227')">
              <span class="btn-icon">💬</span>
              <span>WhatsApp</span>
            </button>
            <button class="action-btn call-btn" onclick="contactCall('919876543227')">
              <span class="btn-icon">📞</span>
              <span>Call</span>
            </button>
          </div>
        ...
        <div class="vehicle-card" data-vehicle="sedan" data-verified="true" data-availability="now">
          <div class="card-header">
            <div class="user-info">
              <div class="user-avatar">
                <img src="https://ui-avatars.com/api/?name=Amit+Joshi&background=ff9900&color=1a1a1a" alt="Driver">
              </div>
              <div class="user-details">
                <div class="user-name-row">
                  <h4 class="user-name">Amit Joshi</h4>
                  <span class="verified-badge">✓ Verified</span>
                </div>
                <div class="id-rating-row">
                  <p class="vehicle-location">📍 Surat</p>
          ...
          <div class="card-body">
            ...
            <div class="vehicle-info-container">
              <div class="vehicle-main-info">
                <span class="vehicle-type-icon">🚗</span>
                <div class="vehicle-details">
                  <p class="vehicle-name">Skoda Rapid</p>
                  <div class="vehicle-specs">
                    <span class="spec-chip">⚡ Diesel</span>
                    <span class="spec-chip">👥 5 Seats</span>
                    <span class="spec-chip">❄️ AC</span>
            ...
            <div class="vehicle-notes-card">
              <span class="notes-label">About:</span>
              <span>Premium sedan with excellent comfort and performance. Professional driver available.</span>
            </div>
          </div>

          <div class="card-footer">
            <button class="action-btn whatsapp-btn" onclick="contactWhatsApp('919876543228')">
              <span class="btn-icon">💬</span>
              <span>WhatsApp</span>
            </button>
            <button class="action-btn call-btn" onclick="contactCall('919876543228')">
              <span class="btn-icon">📞</span>
              <span>Call</span>
            </button>
          </div>
        ...
        <div class="vehicle-card" data-vehicle="hatchback" data-verified="true" data-availability="later">
          <div class="card-header">
            <div class="user-info">
              <div class="user-avatar">
                <img src="https://ui-avatars.com/api/?name=Deepak+Gupta&background=ff9900&color=1a1a1a" alt="Driver">
              </div>
              <div class="user-details">
                <div class="user-name-row">
                  <h4 class="user-name">Deepak Gupta</h4>
                  <span class="verified-badge">✓ Verified</span>
                </div>
                <div class="id-rating-row">
                  <p class="vehicle-location">📍 Ahmedabad</p>
          ...
          <div class="card-body">
            ...
            <div class="vehicle-info-container">
              <div class="vehicle-main-info">
                <span class="vehicle-type-icon">🚗</span>
                <div class="vehicle-details">
                  <p class="vehicle-name">Maruti Baleno</p>
                  <div class="vehicle-specs">
                    <span class="spec-chip">⚡ Petrol</span>
                    <span class="spec-chip">👥 5 Seats</span>
                    <span class="spec-chip">❄️ AC</span>
            ...
            <div class="vehicle-notes-card">
              <span class="notes-label">About:</span>
              <span>Sleek and modern hatchback perfect for daily commuting. Clean vehicle with good mileage.</span>
            </div>
          </div>

          <div class="card-footer">
            <button class="action-btn whatsapp-btn" onclick="contactWhatsApp('919876543229')">
              <span class="btn-icon">💬</span>
              <span>WhatsApp</span>
            </button>
            <button class="action-btn call-btn" onclick="contactCall('919876543229')">
              <span class="btn-icon">📞</span>
              <span>Call</span>
            </button>
          </div>
...
function switchView(view) {
  ...
  if (view === "bookings") {
    ...
    setTimeout(() => {
      ...
      vehiclesView.style.opacity = "0";
      setTimeout(() => {
        vehiclesView.style.opacity = "1";
        // Load vehicles when vehicles view becomes visible
        console.log("🔄 [switchView] Switching to vehicles view, loading vehicles...");
        const vehiclesCards = document.getElementById("vehiclesCards");
        console.log("🔍 [switchView] vehiclesCards container found:", !!vehiclesCards);
        if (vehiclesCards) {
          // Check if API service is available, then load vehicles
          if (typeof apiService !== "undefined") {
            console.log("✅ [switchView] API service available, calling loadVehiclesFromAPI()");
            loadVehiclesFromAPI();
          } else {
            console.log("⚠️ [switchView] API service not loaded, loading it first...");
            // Load API service first, then load vehicles
            const script = document.createElement("script");
            script.src = "../js/services/api.js";
            script.onload = () => {
              console.log("✅ [switchView] API service loaded, calling loadVehiclesFromAPI()");
              loadVehiclesFromAPI();
            };
            script.onerror = () => {
              console.error("❌ [switchView] Failed to load API service");
            };
            document.head.appendChild(script);
          }
        } else {
          console.error("❌ [switchView] vehiclesCards container not found!");
        }
      }, 10);
      window.scrollTo({ top: 0, behavior: "smooth" });
    }, 200);
...
function filterVehicles(filterType) {
  ...
  cards.forEach((card) => {
    if (filterType === "all") {
      ...
      if (card.dataset.availability === "now") {
        card.style.display = "block";
      } else {
        card.style.display = "none";
      }
