...
if (Test-Path (Join-Path $PSScriptRoot "server.js")) {
    Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$PSScriptRoot'; Write-Host '🌐 Frontend Server Starting...' -ForegroundColor Green; node server.js"
    Write-Host "✅ Frontend server starting in new window`n" -ForegroundColor Green
} else {
    Write-Host "❌ server.js not found`n" -ForegroundColor Red
}
