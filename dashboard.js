...
function updateMainContent(tabName) {
  ...
  if (tabName === "available") {
    setTimeout(() => {
      ...
      const isVehiclesViewActive =
        vehiclesView && !vehiclesView.classList.contains("hidden");

      // Ensure API service is loaded first
      if (typeof apiService !== "undefined") {
        if (isVehiclesViewActive) {
          loadVehiclesFromAPI();
        } else {
          loadBookingsFromAPI();
        }
...
async function loadBookingsFromAPI() {
  ...
  try {
    ...
    console.log("📋 Bookings extracted:", bookings.length, bookings);
    ...
    container.innerHTML = `
      <div class="error-bookings" style="text-align: center; padding: 60px 20px; color: #f44336;">
        ...
        <button onclick="loadBookingsFromAPI()" style="padding: 12px 24px; background: #ff9900; color: white; border: none; border-radius: 8px; cursor: pointer; font-size: 16px;">
          Retry
        </button>
      </div>
    `;
...
async function loadVehiclesFromAPI() {
  ...
  try {
    ...
    // Get current filters (can be enhanced later)
    const filters = {};
    ...
    console.log("🚗 Vehicles extracted:", vehicles.length, vehicles);
...
function createVehicleCard(vehicle) {
  ...
  // Vehicle ID (for data attributes and display)
  const vehicleId = vehicle._id?.toString() || "";
  // Vehicle display ID (similar to booking ID)
  ...
  return `
    <div class="vehicle-card${restrictedClass}" 
         data-vehicle="${vehicle.vehicleType || ""}" 
      ...
      <div class="card-header">
        <div class="user-info">
          ...
          <div class="user-details">
            ...
            <div class="id-rating-row">
              <p class="booking-id">#${vehicleDisplayId}</p>
              <div class="rating-section">
                <span class="gold-stars">⭐⭐⭐⭐⭐</span>
                <span class="review-count">(0 reviews)</span>
              </div>
      ...
      <div class="card-body">
        <div class="pickup-row">
          <span class="pickup-label">Available:</span>
          <span class="pickup-value">${formattedDate} • ${formattedTime}</span>
          <span class="upload-time">${timeAgo}</span>
        </div>
